...
export function AddProjectDialog({ onClose }: { onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60" onClick={onClose}>
      <div
        className="w-[480px] rounded-lg border border-[var(--color-panel-edge)] bg-[var(--color-panel-raised)] p-5"
      ...
      >
        ...
        <label className="mb-3 block">
          <span className="mb-1 block text-xs text-[var(--color-ink-dim)]">Folder path</span>
          <input
            autoFocus
            value={path}
            onChange={(e) => setPath(e.target.value)}
            placeholder="C:\Users\you\Documents\Projects\my-app"
            className="w-full rounded border border-[var(--color-panel-edge)] bg-[var(--color-bezel)] px-3 py-2 text-sm outline-none focus:border-[var(--color-phosphor-dim)]"
          />
        </label>
        <label className="mb-4 block">
          <span className="mb-1 block text-xs text-[var(--color-ink-dim)]">Name (optional)</span>
          <input
            value={name}
